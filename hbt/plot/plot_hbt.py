...
def hbt():
    ...
    row_labels = '$R_o$ [fm]', '$R_s$ [fm]', '$R_l$ [fm]'
    HBT_radii = 'Ro', 'Rs', 'Rl'
    ...
    for col, centrality, exp in zip(range(3), centralities, experimental):    
        ...
        try:
            ...
            mt = np.sqrt(R_HQCD['kt_list']**2 + mass**2)
            ratio = R_HQCD['Ro']/R_HQCD['Rs']
            error = ratio*np.sqrt((R_HQCD['dRo']/R_HQCD['Ro'])**2 + (R_HQCD['dRs']/R_HQCD['Rs'])**2)
            ...
            mt = np.sqrt(R_WB['kt_list']**2 + mass**2)
            ratio = R_WB['Ro']/R_WB['Rs']
            error = ratio*np.sqrt((R_WB['dRo']/R_WB['Ro'])**2 + (R_WB['dRs']/R_WB['Rs'])**2)
            ...
            mt = np.sqrt(R_S95['kt_list']**2 + mass**2)
            ratio = R_S95['Ro']/R_S95['Rs']
            error = ratio*np.sqrt((R_S95['dRo']/R_S95['Ro'])**2 + (R_S95['dRs']/R_S95['Rs'])**2)
