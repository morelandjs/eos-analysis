...
def v3():
    ...
    for ax, centrality, label in zip(axes, centralities, labels):
        try:
            ...
            ax.plot(pi['pt'], pi['v3'], linewidth=0.75, color=plt.cm.Blues(0.6), label='$\pi^\pm$')
            ax.fill_between(pi['pt'], pi['v3'] - pi['dv3'], pi['v3'] + pi['dv3'], color=plt.cm.Blues(0.6), alpha=0.5, linewidth=0)
           
            ax.plot(k['pt'], k['v3'], linewidth=0.75, color=plt.cm.Oranges(0.6), label='$K^\pm$')
            ax.fill_between(k['pt'], k['v3'] - k['dv3'], k['v3'] + k['dv3'], color=plt.cm.Oranges(0.6), alpha=0.5, linewidth=0)

            ax.plot(p['pt'], p['v3'], linewidth=0.75, color=plt.cm.Greens(0.6), label='$p$')
            ax.fill_between(p['pt'], p['v3'] - p['dv3'], p['v3'] + p['dv3'], color=plt.cm.Greens(0.6), alpha=0.5, linewidth=0)
    ...
    for ax, centrality in zip(axes, centralities):
        try:
            ...
            pt = pi_HotQCD['pt']
            ratio = pi_WB['v3']/pi_HotQCD['v3']
            err = ratio*np.sqrt((pi_WB['dv3']/pi_WB['v3'])**2 + (pi_HotQCD['dv3']/pi_HotQCD['v3'])**2)
            ...
            pt = k_HotQCD['pt']
            ratio = k_WB['v3']/k_HotQCD['v3']
            err = ratio*np.sqrt((k_WB['dv3']/k_WB['v3'])**2 + (k_HotQCD['dv3']/k_HotQCD['v3'])**2)
            ...
            pt = p_HotQCD['pt']
            ratio = p_WB['v3']/p_HotQCD['v3']
            err = ratio*np.sqrt((p_WB['dv3']/p_WB['v3'])**2 + (p_HotQCD['dv3']/p_HotQCD['v3'])**2)
    ...
    for ax, centrality in zip(axes, centralities):
        try:
            ...
            pt = pi_HotQCD['pt']
            ratio = pi_s95['v3']/pi_HotQCD['v3']
            err = ratio*np.sqrt((pi_s95['dv3']/pi_s95['v3'])**2 + (pi_HotQCD['dv3']/pi_HotQCD['v3'])**2)
            ax.plot(pt, ratio, linewidth=0.75, color=plt.cm.Blues(0.6))
            ax.fill_between(pt, ratio - err , ratio + err, color=plt.cm.Blues(0.6), alpha=0.5, linewidth=0)

            pt = k_HotQCD['pt']
            ratio = k_s95['v3']/k_HotQCD['v3']
            err = ratio*np.sqrt((k_s95['dv3']/k_s95['v3'])**2 + (k_HotQCD['dv3']/k_HotQCD['v3'])**2)
            ax.plot(pt, ratio, linewidth=0.75, color=plt.cm.Oranges(0.6))
            ax.fill_between(pt, ratio - err , ratio + err, color=plt.cm.Oranges(0.6), alpha=0.5, linewidth=0)
           
            pt = p_HotQCD['pt']
            ratio = p_s95['v3']/p_HotQCD['v3']
            err = ratio*np.sqrt((p_s95['dv3']/p_s95['v3'])**2 + (p_HotQCD['dv3']/p_HotQCD['v3'])**2)
            ax.plot(pt, ratio, linewidth=0.75, color=plt.cm.Greens(0.6))
            ax.fill_between(pt, ratio - err , ratio + err, color=plt.cm.Greens(0.6), alpha=0.5, linewidth=0)
